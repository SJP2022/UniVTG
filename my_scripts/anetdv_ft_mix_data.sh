#!/bin/bash
#SBATCH -o out/anetdv_ft_out/anetdv_ft_mix_data.out
#SBATCH --partition=fvl       # 作业提交的指定分区;
#SBATCH --qos=high            # 指定作业的QOS;
#SBATCH -J anetdv_ft_mix_data # 作业在调度系统中的作业名;
...
eval_path=data/anet/metadata/val_1.jsonl
eval_split_name=val
feat_root=data/anet
feat_root_val=data/anet
mix_path=data/anetdv_new/metadata/train.jsonl
feat_root_mix=data/anetdv_new
...
if [[ ${t_feat_type} == "clip" ]]; then
  ...
  t_feat_dir_mix=${feat_root_mix}/txt_clip
  t_feat_dim=512
else
...
--out ${out} \
--eval_init ${@:1}  \
> out/anetdv_ft_out/anetdv_ft_mix_data.out
...
dset_name=anet
anno_type=anetdv
out=anetdv_ft_mix_data_inference
clip_length=2
...
resume=results/mr-anetdv/qvhl-anetdv_ft_mix_data/model_best.ckpt
...
######## data paths
train_path=data/anet/metadata/train.jsonl
eval_path=data/anet/metadata/val_2.jsonl
eval_split_name=val
feat_root=data/anet
feat_root_val=data/anet
mix_path=data/anetdv_new/metadata/train.jsonl
...
if [[ ${t_feat_type} == "clip" ]]; then
  t_feat_dir=${feat_root}/txt_clip
  t_feat_dir_val=${feat_root_val}/txt_clip_test
  t_feat_dir_mix=${feat_root_mix}/txt_clip
  t_feat_dim=512
else
  echo "Wrong arg for t_feat_type."
...
--out ${out} \
--eval_init ${@:1}  \
> out/anetdv_ft_out/anetdv_ft_mix_data_inference.out