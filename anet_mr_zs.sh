#!/bin/bash
#SBATCH -o anetdv_mr_pt_val.out   # 脚本执行的输出将被保存在my.%j.out文件下，%j表示作业号;
#SBATCH --partition=fvl       # 作业提交的指定分区;
#SBATCH --qos=medium          # 指定作业的QOS;
#SBATCH -J mr_pt_val                 # 作业在调度系统中的作业名;
#SBATCH --nodes=1             # 申请节点数为1;
#SBATCH --gres=gpu:1          # 申请GPU数量为1;
#SBATCH --mem=24G             # 申请内存为24G;
#SBATCH --time=48:00:00       # 作业运行的最长时间为2天
source activate univtg        # 激活的conda环境为cola;
...
#resume=results/mr-anet/qvhl-slowfast_clip-clip-2024_03_10_00/model_best.ckpt
...
######## data paths
dset_path=anetdv
train_path=data/${dset_path}/metadata/train.jsonl
eval_path=data/${dset_path}/metadata/val.jsonl
eval_split_name=val
feat_root=data/${dset_path}
